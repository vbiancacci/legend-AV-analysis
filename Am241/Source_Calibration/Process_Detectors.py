        ...
        for detector in detectors:
            ...
            if Gamma_line_count_MC == True:
                ...
                for FCCD in FCCD_list:
                    ...
                    for DLF in DLF_list:
                        MC_id=detector+"-"+source+"-top-0r-198z_"+smear+"_"+TL_model+"_FCCD"+str(FCCD)+"mm_DLF"+str(DLF)+"_fracFCCDbore"+str(frac_FCCDbore)
                        sim_path="/lfs/l1/legend/detector_char/enr/hades/simulations/legend-g4simple-simulation/simulations/"+detector+"/"+source+"/top_0r_198z/hdf5/AV_processed/"+MC_id+".hdf5"
                        os.system("python3 "+CodePath+"/GammaLine_Counting_Am241.py --sim "+detector+" "+sim_path+" "+MC_id+" "+source)

            #=============Calculate FCCD===============
            if Calculate_FCCD == True:

                MC_id=detector+"-"+source+"-top-0r-198z"
            ...
            if Gamma_line_count_MC_bestfitFCCD == True:
                ...
                if cuts == "False":
                    with open(CodePath+"/FCCD/"+source+"/FCCD_data_"+detector+"/"+source+"-top-0r-198z_"+smear+"_"+TL_model+"_fracFCCDbore"+str(frac_FCCDbore)+"_"+energy_filter+"_run"+str(run)+".json") as json_file:
                        FCCD_data = json.load(json_file)
                else:
                    with open(CodePath+"/FCCD/"+source+"/FCCD_data_"+detector+"/"+source+"-top-0r-198z_"+smear+"_"+TL_model+"_fracFCCDbore"+str(frac_FCCDbore)+"_"+energy_filter+"_run"+str(run)+"_cuts.json") as json_file:
                        FCCD_data = json.load(json_file)
            ...
            if PlotSpectra == True:
                ...
                if cuts == "False":
                    with open(CodePath+"/FCCD/FCCD_data_"+detector+"/"+source+"-top-0r-198z_"+smear+"_"+TL_model+"_fracFCCDbore"+str(frac_FCCDbore)+"_"+energy_filter+"_run"+str(run)+".json") as json_file:
                        FCCD_data = json.load(json_file)
                else:
                    with open(CodePath+"/FCCD/FCCD_data_"+detector+"/"+source+"top-0r-198z_"+smear+"_"+TL_model+"_fracFCCDbore"+str(frac_FCCDbore)+"_"+energy_filter+"_run"+str(run)+"_cuts.json") as json_file:
                        FCCD_data = json.load(json_file)
                FCCD = round(FCCD_data["FCCD"],2)
